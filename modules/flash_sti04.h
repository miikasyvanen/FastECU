...
private:
    ...
    int init_flash_sti04(FileActions::EcuCalDefStructure *ecuCalDef, QString cmd_type);
    ...
    int connect_bootloader_subaru_denso_can_04_32bit();
    int upload_kernel_subaru_denso_can_04_32bit(QString kernel);
    int read_mem_subaru_denso_can_32bit(FileActions::EcuCalDefStructure *ecuCalDef, uint32_t start_addr, uint32_t length);
    int write_mem_subaru_denso_can_32bit(FileActions::EcuCalDefStructure *ecuCalDef, bool test_write);
    int get_changed_blocks_can_32bit(const uint8_t *src, int *modified);
    int check_romcrc_32bit_can(const uint8_t *src, uint32_t start_addr, uint32_t len, int *modified);
    int flash_block_can_32bit(const uint8_t *src, uint32_t start, uint32_t len);
    int reflash_block_can_32bit(const uint8_t *newdata, const struct flashdev_t *fdt, unsigned blockno, bool test_write);
