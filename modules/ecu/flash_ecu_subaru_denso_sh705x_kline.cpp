 ...
 * @return success
 */
int FlashEcuSubaruDensoSH705xKline::upload_kernel(QString kernel, uint32_t kernel_start_addr)
{
    QFile file(kernel);
...
QByteArray FlashEcuSubaruDensoSH705xKline::send_sid_bf_ssm_init()
{
    ...
    while (received == "" && loop_cnt < 5)
    {
        ...
        emit LOG_D("Sent: " + parse_message_to_hex(output), true, true);
        delay(500);
        received = serial->read_serial_data(8, serial_read_long_timeout);
        while(received.length() != (uint8_t)received.at(3) && read_cnt < 100)
        {
 ...
 * @return received response
 */
QByteArray FlashEcuSubaruDensoSH705xKline::send_sid_34_request_upload(uint32_t dataaddr, uint32_t datalen)
{
    QByteArray output;
    QByteArray received;
    QByteArray msg;
 ...
 * @return received response
 */
QByteArray FlashEcuSubaruDensoSH705xKline::send_sid_36_transferdata(uint32_t dataaddr, QByteArray buf, uint32_t len)
{
    QByteArray output;
    QByteArray received;
    ...
    for (blockno = 0; blockno <= maxblocks; blockno++)
    {
        ...
        output = add_ssm_header(output, tester_id, target_id, false);
        serial->write_serial_data_echo_check(output);
        QByteArray header = output;
        header.remove(8, header.length()-8);
        emit LOG_D("Sent: " + parse_message_to_hex(header), true, true);
        received = serial->read_serial_data(6, serial_read_extra_long_timeout);
...
QByteArray FlashEcuSubaruDensoSH705xKline::send_sid_31_start_routine()
{
    QByteArray output;
    QByteArray received;
    uint8_t loop_cnt = 0;
