...
private:
    ...
    int init_flash_mitsu_kline();

    int connect_bootloader_subaru_ecu_mitsu_kline();
    //int upload_kernel_subaru_denso_can_02_32bit(QString kernel, uint32_t kernel_start_addr);
    int read_mem_subaru_ecu_mitsu_kline(uint32_t start_addr, uint32_t length);
    int write_mem_subaru_ecu_mitsu_kline(bool test_write);
    ...
    int reflash_block_subaru_ecu_mitsu_kline(const uint8_t *newdata, const struct flashdev_t *fdt, unsigned blockno, bool test_write);
    //int erase_subaru_ecu_mitsu_kline();
    ...
    QByteArray send_subaru_mitsu_sid_bf_ssm_init();
    QByteArray send_subaru_mitsu_sid_a0_block_read(uint32_t dataaddr, uint32_t datalen);
    QByteArray send_subaru_mitsu_sid_b8_byte_read(uint32_t dataaddr);
    QByteArray send_subaru_mitsu_sid_81_start_communication();
    QByteArray send_subaru_mitsu_sid_83_request_timings();
    ...
    QByteArray subaru_ecu_mitsu_generate_kline_seed_key(QByteArray requested_seed);
    QByteArray subaru_ecu_mitsu_calculate_seed_key(QByteArray requested_seed, const uint16_t *keytogenerateindex, const uint8_t *indextransformation);
    QByteArray subaru_ecu_mitsu_encrypt_32bit_payload(QByteArray buf, uint32_t len);
    QByteArray subaru_ecu_mitsu_decrypt_32bit_payload(QByteArray buf, uint32_t len);
    QByteArray subaru_ecu_mitsu_calculate_32bit_payload(QByteArray buf, uint32_t len, const uint16_t *keytogenerateindex, const uint8_t *indextransformation);
