...
unsigned long SerialPortActions::read_vbatt()
{
    if (isDirectConnection())
    {
        if (!this->get_is_comm_busy() && !get_read_vbatt())
            vBatt = serial_direct->read_vbatt();
        else
            set_read_vbatt(true);
    }
